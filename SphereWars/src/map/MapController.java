...
public class MapController {
	...
	//Posici贸n en bloque dentro del mapa, y pixel dentro del bloque
	private int pos_block;
	private int pixel_block;
	//Tama駉 que va a ocupar cada bloque en pantalla
	...
	public MapController(int width, int height){
		...
		//Se suma 1 ya que la divisi贸n entre enteros si no es exacta desprecia los decimales, asi ocupa todo el ancho
	...
	public void draw2D(Graphics2D g, int x_ori, int y_ori, boolean not_pause) {
		int width_map1;
		int width_map2 = 0;
		//Se suma 1 bloque al 煤ltimo mapa para evitar que aparezca de repente durante el desplazamiento
	...
	public double move() {
		...
		//TODO: usar speedLow y speedHigh, los valores fijos no funcionan bien si cambia la resoluci贸n de pantalla del juego
		int speed = speedLow;
		pixel_block += speed;
	...
	public int removeTresure(Sphere player, int direction, int x_ori, int y_ori) {
		...
		//Apa帽o, Richard es malvado y carga el mapa con las "y" invertidas
